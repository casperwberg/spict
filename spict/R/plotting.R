...
#' @name plotspict.seasonP
#' @title Plot the mean B cycle
#' @details If seasonal data are available the seasonal cycle in the biomass production can be estimated. This function plots this mean B cycle.
#' @param rep A result report as generated by running fit.spict.
#' @param stamp Stamp plot with this character string.
#' @return Nothing.
#' @export
...
plotspict.seasonP <- function(rep, stamp=get.version()){
    if (!'par.fixed' %in% names(rep)){
        stop('Input object was not a valid output from fit.spict()!')
    }
    if (!'sderr' %in% names(rep) & 'logphiP' %in% names(rep$par.fixed)){
        jan <- as.POSIXct("2015-01-01 00:00:01 UTC", tz='UTC')
        apr <- jan+(31+28+31)*24*60*60
        jul <- apr+(30+31+30)*24*60*60
        oct <- jul+(31+31+30)*24*60*60
        logB <- get.par('logB', rep)
        meanlogB <- mean(logB[, 2])
        meanlogB <- 0 # Don't add mean B
        logphiP <- get.par('logphiP', rep)
        naflag <- any(is.na(logphiP[, 1]))
...
=======
        ...
        if(rep$inp$seasontypeP==1){
            ...
            yest <- rep(logphiP[,2], each=rep$inp$nseasonsP)
            test <- seq(0,1,length=length(y))            
            ylim <- range(yest, na.rm=TRUE)
        ...
        if (rep$inp$seasontypeP==1){        
            plot(test, yest, typ='n', xaxt='n', xlab='Time of year', ylab='Log seasonal factor',
                 main=paste('Seasonal production - number of steps:',length(rep$inp$parlist$logphiP)), ylim=ylim)
        }
        if (rep$inp$seasontypeP==2){        
            plot(t, y, typ='n', xaxt='n', xlab='Time of year', ylab='Log seasonal spline',
                 main=paste('Seasonal production - spline order:',rep$inp$splineorderP), ylim=ylim)
            cicol2 <- rgb(0, 0, 1, 0.1)
            cicol3 <- rgb(0, 0, 1, 0.2)
            polygon(c(t, rev(t)), c(sssl, rev(sssu)), col=cicol2, border=cicol2)
            if (!naflag){
